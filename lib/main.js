...
var newPath = process.env['PATH'].replace(/[^;]+?(Chocolatey|CMake|mingw64|OpenSSL|Strawberry|tools\\php)[^;]*;/g, '');
...
async function installRubyWin() {
  ...
  if (rubyIdx === 0) {
    ...
    fs.renameSync(`C:\\${ri2FldrLwr}`, `C:\\${rubyVers}`);
    // core.exportVariable('SSL_CERT_FILE'   , `C:/${rubyVers}/ssl/cert.pem`);
    // core.exportVariable('DEFAULT_CERT_DIR', `C:/${rubyVers}/ssl/certs`);
  } else if (rubyIdx === rubyHeadIdx) {
